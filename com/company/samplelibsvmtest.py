...
data, outcome = sklearn.datasets.load_svmlight_file('/home/raghunandangupta/Downloads/soc_gen_data/cde.txt', multilabel=True, zero_based=False)
...
df = pd.SparseDataFrame([ pd.SparseSeries(data[i].toarray().ravel()) 
                              for i in np.arange(outcome.__len__()) ])
